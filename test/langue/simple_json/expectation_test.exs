defmodule LangueTest.Formatter.SimpleJson.Expectation do
  ...
  defmodule InterpolationValues do
    ...
    def entries do
      [
        %Entry{comment: "", index: 1, key: "single", value: "Hello, {{username}}.", interpolations: ~w({{username}})},
        %Entry{comment: "", index: 2, key: "multiple", value: "Hello, {{firstname}} {{lastname}}.", interpolations: ~w({{firstname}} {{lastname}})},
        %Entry{comment: "", index: 3, key: "duplicate", value: "Hello, {{username}}. Welcome back {{username}}.", interpolations: ~w({{username}} {{username}})},
        %Entry{comment: "", index: 4, key: "empty", value: "Hello, {{}}.", interpolations: ~w({{}})}
      ]
    end
  end
end
